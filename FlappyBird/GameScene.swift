...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            //効果音を鳴らす
            let playsound = SKAction.playSoundFileNamed("effect_item.mp3", waitForCompletion: false)
            self.run(playsound)
            //itemを消す
    ...
    func setupItem(){
        let itemTexture = SKTexture(imageNamed: "azarashi")
        ...
        let createItemAnimation = SKAction.run({
            ...
            self.item.position = CGPoint(x:self.frame.size.width, y:random_y)
            self.item.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width:self.item.size.width, height: self.item.size.height))
    ...
    func setupWall(){
        ...
        let createWallAnimation = SKAction.run({
            ...
            scoreNode.position = CGPoint(x:upper.size.width + birdSize.width / 2, y: under_wall_y + slit_length / 2)
            scoreNode.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width:upper.size.width, height: self.frame.size.height))
            //scoreNode.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width:upper.size.width, height: slit_length))
